...
| Deployment/release              | Present  | `docs/deployment/cloudflare-pages.md`, `.github/workflows/deploy-cloudflare.yml`       | Keep Cloudflare and Supabase secret names current.                                |
...
| Local quality gates             | Present  | `.husky/pre-commit`, `.husky/pre-push`, `package.json`                                 | Pre-push runs lint, typecheck, unit, architecture, and build checks.              |
| CI                              | Present  | `.github/workflows/ci.yml`                                                             | CI mirrors format, lint, typecheck, unit, architecture, build, and E2E checks.    |
| Security/public readiness       | Present  | `scripts/check-public-readiness.mjs`, `.gitignore`, `README.md`, `SECURITY.md`         | Keep secret, workflow, path, and boundary checks current.                         |
...
| Dependency hygiene              | Present  | `package-lock.json`, `npm audit` command in `README.md`                                | Run `npm audit --audit-level=moderate` before push.                               |
